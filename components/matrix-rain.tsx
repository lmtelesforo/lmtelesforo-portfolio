...
export default function MatrixRain() {
  ...
  useEffect(() => {
    ...
    const initializeDrops = (width: number, height: number) => {
      const drops: Drop[] = []
      const dropCount = Math.floor(width / 22) // Slightly less dense than /18
    ...
    // Set up event listeners (fallback)
    window.addEventListener("resize", handleResize)
    // Small delay to ensure parent dimensions are final
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Make mouse position relative to canvas instead of window
      setMousePos({ 
        ...
        drop.characters.forEach((char, charIndex) => {
          ...
          if (y < dimensions.height && y > 0) {
            ...
            if (distance < influenceRadius) {
              ...
              // Vibrant electric purple for mouse interaction (#7980DA base)
              const r = Math.floor(121 + 60 * intensity) 
              const g = Math.floor(128 + 60 * intensity) 
              const b = Math.floor(218 + 37 * intensity) 
              ctx.fillStyle = `rgb(${r}, ${g}, ${b})`
            } else {
              // Default color: vibrant brand purple with more visible opacities
              const opacity = charIndex === 0 ? 0.6 : (1 - charIndex / drop.characters.length) * 0.3
              ctx.fillStyle = `rgba(121, 128, 218, ${opacity})`
            }
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 z-0 pointer-events-none opacity-30 mix-blend-screen" aria-label="Japanese character rain animation" />
}
